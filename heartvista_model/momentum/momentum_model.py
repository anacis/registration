...
class MomentumModel(torch.nn.Module):
    ...
    def __init__(self, network_class, feature_dim=128, max_buffer_size=65536, momentum=0.99975,
        ...
        self.network = network_class(num_classes=feature_dim, input_f= 1 if magnitude else 2).to(device)
        self.target_network = network_class(num_classes=feature_dim, input_f = 1 if magnitude else 2).to(device)
    ...
    def update_memory_bank(self, embeddings,
        ...
        """
        embeddings_flat = embeddings.contiguous().view((-1, self.feature_dim))
