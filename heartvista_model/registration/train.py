...
class Trainer:
    ...
    def get_transforms(self):        
        if self.args.shape == "perspective":
            self.shape_transform = transforms.Compose([transforms.RandomPerspective(distortion_scale=0.6, p=1.0)]) #TODO: might need bilinear interpolation
        else:
            deg = 20
            x, y = 0.1, 0.1
            self.shape_transform = transforms.Compose([transforms.RandomAffine(degrees = deg, translate=(x, y))])#, interpolation=InterpolationMode.BILINEAR)])
